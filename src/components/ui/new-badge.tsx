...
const NewBadge = ({ show, className, size = 'sm' }: NewBadgeProps) => {
  ...
  const getBadgeStyles = () => {
    ...
    const styleVariants = {
      pill: "rounded-full",
      dot: "rounded-full w-3 h-3 p-0 text-[0px]",
      outline: "border-2 bg-transparent text-current"
    };

    return cn(
      baseStyles,
      sizeStyles[size],
      config.badgeStyle === 'dot' ? 'w-3 h-3 p-0 text-[0px]' : colorStyles[config.badgeColor],
      styleVariants[config.badgeStyle],
      config.badgeStyle === 'outline' && `border-${config.badgeColor}-500 text-${config.badgeColor}-500`,
